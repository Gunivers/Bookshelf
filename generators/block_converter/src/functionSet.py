...
def generate(path, function, branch=3, objectList = None, verbose=False, depth=0):
    ...
    # Create folders
    if not os.path.exists(f"{path}/{mcfunctionName}/leaves/"): os.makedirs(f"{path}/{mcfunctionName}/leaves/")
    if not os.path.exists(f"{path}/{mcfunctionName}/nodes/"):  os.makedirs(f"{path}/{mcfunctionName}/nodes/")

    # Creating new branches if their is lot of leaves (= number of objects in the list)
    if len(objectList) > 2 * branch:

        # Opening MCfunction's file
        if depth == 0:                            mcfunction = open(f"{path}/{mcfunctionName}.mcfunction", "w+")
        else:
            if function == "block_id_to_item_id": mcfunction = open(f"{path}/{mcfunctionName}/nodes/{objectList[0].firstBlockID}-{objectList[-1].lastBlockID}.mcfunction", "w+")
            else:                                 mcfunction = open(f"{path}/{mcfunctionName}/nodes/{objectList[0].id}-{objectList[-1].id}.mcfunction", "w+")
        ...
        for i in range(branch):  
            ...
            if function == "block_id_to_item_id": mcfunction.write(f"execute if score @s {scoreboard} matches {group[i][0].block.firstBlockID}..{group[i][-1].block.lastBlockID} run function {module}:{mcfunctionName}/{leavesOrNodes}/{group[i][0].block.firstBlockID}-{group[i][-1].block.lastBlockID}\n")
            else:                                 mcfunction.write(f"execute if score @s {scoreboard} matches {group[i][0].id}..{group[i][-1].id} run function {module}:{mcfunctionName}/{leavesOrNodes}/{group[i][0].id}-{group[i][-1].id}\n")

    else:  # If the objectList is too small to create new nodes

        # Place the mcfunction in the leaves folder
        if function == "block_id_to_item_id": mcfunction = open(f"{path}/{mcfunctionName}/leaves/{objectList[0].firstBlockID}-{objectList[-1].lastBlockID}.mcfunction", "w+")
        else:                                 mcfunction = open(f"{path}/{mcfunctionName}/leaves/{objectList[0].id}-{objectList[-1].id}.mcfunction", "w+")
