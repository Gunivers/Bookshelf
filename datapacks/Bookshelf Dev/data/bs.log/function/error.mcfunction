...
data modify storage bs:data log.ctx set from storage bs:const log.messages[{default: true}].format.error
$data modify storage bs:data log.ctx set from storage bs:const log.messages[{namespaces: ["$(namespace)"]}].format.error
