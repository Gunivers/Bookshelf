...
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run function bs_wip:math/trig/arctan
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run scoreboard players operation @s bs.plot.y = @s bs.res0
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run scoreboard players operation @s bs.plot.y *= 5 bs.const
...
summon minecraft:armor_stand 0.5 97 0.5 {Tags:["bs","bs.sandbox.plot.legend.new"],CustomName:'{"text":"-45°"}',CustomNameVisible:1b,NoGravity:1,Invisible:1,Marker:1}
