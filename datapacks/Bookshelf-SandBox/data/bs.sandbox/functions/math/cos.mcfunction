...
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run scoreboard players operation @s bs.in.0 *= 180 bs.const
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run scoreboard players operation @s bs.in.0 /= 1000 bs.const
execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run function bs.math:trig/cos
#execute as @e[tag=bs.sandbox.plot,tag=!bs.sandbox.plot.drawed] run tellraw @a ["",{"text":"Debug | bs.sandbox:math/plot/cos","color":"green"},{"text":"\n \u0020 Input : "},{"score":{"name":"@s","objective":"bs.in.0"}},{"text":" Output : "},{"score":{"name":"@s","objective":"bs.out.0"}}]
...
summon minecraft:armor_stand 0.5 97 0.5 {Tags:["bs","bs.sandbox.plot.legend.new"],CustomName:'{"text":"cos(x)"}',CustomNameVisible:1b,NoGravity:1,Invisible:1,Marker:1}
