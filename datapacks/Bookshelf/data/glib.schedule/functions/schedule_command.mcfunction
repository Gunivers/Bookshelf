...
# OUTPUT: @e[tag=Glib_Cache_Source] bs.res0
...
#		data modify entity @e[tag=Glib_Cache_Source,limit=1] ArmorItems[0].tag.Buffer set value {Command:"say Slime",Timer:5,TimeUnit:"second",bs.id:"T01"}
#		function bs.schedule:schedule_command

# CODE:

execute as @e[tag=Glib_Cache_Source] if data entity @s ArmorItems[0].tag.Buffer.bs.id if data entity @s ArmorItems[0].tag.Buffer.Timer if data entity @s ArmorItems[0].tag.Buffer.TimeUnit if data entity @s ArmorItems[0].tag.Buffer.Command run tag @s add Valid
