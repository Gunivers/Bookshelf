...
execute as @e[tag=Glib_Cache_Head] at @s store result score @s bs.var0 run data get block ~ ~ ~2 RecordItem.tag.ScheduleCommands[0].GameTick 1
execute as @e[tag=Glib_Cache_Head] at @s if score @s bs.var0 = @s bs.var0 run tag @s add isEqual
