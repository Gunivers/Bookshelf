...
execute as @e[tag=Glib,tag=SetRotation,limit=1,sort=nearest] at @s run function bs.orientation:get
scoreboard players operation @s bs.var0 += @e[tag=Glib,tag=SetRotation,limit=1,sort=nearest] bs.ori.h
scoreboard players operation @s bs.var1 += @e[tag=Glib,tag=SetRotation,limit=1,sort=nearest] bs.ori.v
...
execute store result entity @e[type=armor_stand,tag=SetRotation,limit=1] Rotation[1] float 1 run scoreboard players get @s bs.var1
