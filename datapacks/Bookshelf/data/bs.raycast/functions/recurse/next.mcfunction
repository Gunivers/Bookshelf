...
$execute unless score $raycast.block_collision bs.in matches 0 unless block ~ ~ ~ $(ignored_blocks) run function bs.raycast:check/block
$execute unless score $raycast.entity_collision bs.in matches 0 as @e[tag=$(solid_entities),tag=!bs.raycast.omit,dx=0] run function bs.raycast:check/entity
