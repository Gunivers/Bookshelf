...
execute unless predicate bs.interaction:is_listener run function #bs.log:error { namespace: bs.interaction, path: "bs.interaction:event/enter/add", tag: "interaction.enter.add", message: '"The current entity is not a listener."' }
...
execute if data storage bs:ctx _{should_interpret: true} store result score _ bs.ctx run function bs.interaction:event/enter/evaluate_selector with storage bs:ctx _
execute if score _ bs.ctx matches 0 run function #bs.log:error { namespace: bs.interaction, path: "bs.interaction:event/enter/add", tag: "interaction.event.enter.add", message: '["","The selector \\\'",{"nbt":"_.executor","storage":"bs:ctx"},"\\\' is not valid."]' }
...
execute store result storage bs:ctx _.callback_id int 1 run function bs.interaction:event/enter/get_new_id with storage bs:ctx _
tag @s add bs.interaction.listen_enter
tag @s add bs.interaction.hover
function bs.interaction:event/enter/register_function with storage bs:ctx _
