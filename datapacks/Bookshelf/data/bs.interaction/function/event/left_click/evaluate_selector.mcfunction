...
execute if score _ bs.ctx matches 0 run function #bs.log:error { namespace: bs.interaction, path: "bs.interaction:event/left_click/evaluate_selector", tag: "interaction.event.left_click.add", message: '["","The selector \\\'",{"nbt":"_.executor","storage":"bs:ctx"},"\\\' does not point to any entity."]' }
execute if score _ bs.ctx matches 0 run return -1
execute if score _ bs.ctx matches 2.. run function #bs.log:warn { namespace: bs.interaction, path: "bs.interaction:event/left_click/evaluate_selector", tag: "interaction.event.left_click.add", message: '["","The selector \\\'",{"nbt":"_.executor","storage":"bs:ctx"},"\\\' points to multiple entities. Only the first one is selected."]' }
