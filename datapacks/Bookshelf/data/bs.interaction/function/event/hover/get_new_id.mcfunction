...
# Note:
#  Define a custom behavior for the hover event for the current interaction.
#
# Output:
#  - Return a new ID based on the ID of the last registered event incremented by 1.
#  - 0 if no event has been registered yet.

$execute store result score #result bs.data run data get storage bs:data interaction.listener.$(id).hover[-1].id
