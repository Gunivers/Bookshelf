...
execute as @e[predicate=bs.interaction:on_left_click] at @s on target if entity @s[tag=bs.interaction.source] as @e[type=interaction, limit=1, sort=nearest] run tag @s add bs.interaction.target
...
function bs.interaction:on_event/left_click/get_function with storage bs:ctx _
