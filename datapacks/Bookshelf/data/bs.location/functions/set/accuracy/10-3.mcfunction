...
execute if entity @s[type=minecraft:player] as @e[tag=bs.new,limit=1] run function bs.location:set/accuracy/10-3/player

# Non-player
execute if entity @s[type=!minecraft:player] store result entity @s Pos[0] double 0.001 run scoreboard players add @s bs.loc.x 0
...
execute if entity @s[type=!minecraft:player] store result entity @s Pos[2] double 0.001 run scoreboard players add @s bs.loc.z 0
