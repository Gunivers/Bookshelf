...
execute if score #move.y1 bs.data > #move.y2 bs.data positioned ~ ~-1 ~ run function bs.move:collision/recurse/y/nn with storage bs:in move
scoreboard players add #move.y1 bs.data 1000000
scoreboard players add #move.x1 bs.data 1000000
execute if score #move.x1 bs.data < #move.x2 bs.data positioned ~1 ~ ~ run function bs.move:collision/recurse/x/pnn with storage bs:in move
