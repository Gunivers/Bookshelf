...
scoreboard players operation @s bs.var2 > @e[tag=Glib_Pathfind_Rewind_Check] PathCost
scoreboard players operation @s bs.var2 < @e[tag=Glib_Pathfind_Rewind_Check] PathCost

execute as @e[tag=Glib_Pathfind_Rewind_Check] if score @s PathCost = @e[tag=Glib_Pathfind_Rewind_Head,limit=1] bs.var2 run tag @s add Glib_Pathfind_Rewind_Head
...
execute unless entity @e[tag=Glib_Pathfind_Source,distance=..1.5] as @e[tag=Glib_Pathfind_Rewind_Head] at @s run function bs.move:pathfind/child/loop_get_path