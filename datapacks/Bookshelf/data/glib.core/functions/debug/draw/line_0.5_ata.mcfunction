...
execute as @e[tag=glib.debug.draw_line.dest] run function bs.health:safe_kill

scoreboard players set debug.draw_line glib 200
execute unless entity @e[tag=glib.debug.draw_line,distance=..0.5] as @e[tag=glib.debug.draw_line,limit=1,sort=nearest] run function bs.core:debug/draw/line_0.5/child/line_0.5

execute as @e[tag=glib.debug.draw_line,limit=1,sort=nearest] run function bs.health:safe_kill
