...
# Dependencies:
# Note: 
#  Executed when an interaction is leaved, trigger the callbacks linked to the current interaction, defined with the leave/add function.
#
# Input:
#  - bs:in gui.callbacks: The list of callbacks linked to the current interaction for the leave event.
...
execute as @e[tag=bs.gui.executor] run function bs.gui:interaction/on_event/hover/leave/execute_function with storage bs:ctx _
