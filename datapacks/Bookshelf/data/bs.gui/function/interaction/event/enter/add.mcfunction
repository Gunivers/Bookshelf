...
execute unless predicate bs.gui:interaction/is_listener run function #bs.log:error { path: "bs.gui:interaction/event/enter/add", feature: "interaction.event.enter.add", message: '"The current entity is not a listener."'}
...
execute store result storage bs:ctx _.callback_id int 1 run function bs.gui:interaction/event/enter/get_new_id with storage bs:ctx _
tag @s add bs.gui.listen_enter
tag @s add bs.gui.hover
function bs.gui:interaction/event/enter/register_function with storage bs:ctx _