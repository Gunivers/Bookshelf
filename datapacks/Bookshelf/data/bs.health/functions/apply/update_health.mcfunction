...
execute store result storage bs:_ points double 0.00001 run scoreboard players operation #health.points bs.data += @s bs.health.points
...
execute if score @s[tag=!bs.health.skip_tick] bs.health.points matches 1.. run function bs.health:apply/add_health with storage bs:_
execute if score @s[tag=!bs.health.skip_tick] bs.health.points matches ..-1 run function bs.health:apply/remove_health with storage bs:_
scoreboard players reset @s[tag=!bs.health.skip_tick] bs.health.points

tag @s remove bs.health.skip_tick
execute unless entity @s[scores={bs.health.max=0..}] run function bs.health:apply/set_max_health with storage bs:_
