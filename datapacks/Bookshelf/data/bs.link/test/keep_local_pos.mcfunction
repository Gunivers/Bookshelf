...
tp @e[type=minecraft:armor_stand,tag=packtest.1] 0.0 0.0 0.0 52 -12
execute as @e[type=minecraft:armor_stand,tag=packtest.2] run function #bs.link:keep_local_pos

execute as @e[type=minecraft:armor_stand,tag=packtest.1] at @s positioned ^ ^ ^1 run assert entity @e[type=minecraft:armor_stand,tag=packtest.2,distance=..0.002]
