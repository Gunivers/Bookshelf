...
execute as @e[type=minecraft:armor_stand,tag=packtest.1] run function #bs.id:give_suid
scoreboard players operation @e[type=minecraft:armor_stand,tag=packtest.2] bs.link.to = @e[type=minecraft:armor_stand,tag=packtest.1] bs.id
execute as @e[type=minecraft:armor_stand,tag=packtest.2] run function #bs.link:update_link
