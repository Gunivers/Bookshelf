...
# Start Backup
scoreboard players operation backup.link.reverse.oriH glib = @s bs.oriH
# End Backup
...
scoreboard players operation link.reverseLoc.diff glib = @s bs.oriH
execute at @e[tag=bs.id.match,limit=1,sort=nearest] run function bs.orientation:get_h/accuracy/10-3
scoreboard players operation link.reverseLoc.diff glib -= @s bs.oriH

# Calcul difference
scoreboard players operation link.reverseLoc.diff glib -= @s bs.link.rh

# Update link
scoreboard players operation link.reverseLoc.diff glib *= 2 bs.const
scoreboard players operation @s bs.link.rh += link.reverseLoc.diff glib
...
# Start Restore
scoreboard players operation @s bs.oriH = backup.link.reverse.oriH glib
# End Restore
