...
execute if entity @s[tag=bs_Creator_Valid_System] run tag @s remove bs_Creator_system.blackhole


# Non-destructive placement system
function bs.core:menu/creator/place_system/main
execute as @e[tag=bs_Example_System,limit=1,sort=nearest] at @s if block ~ ~ ~ air run setblock ~ ~ ~ minecraft:structure_block[mode=load]{name:"bs.core:example_systems/blackhole",sizeX:2,sizeY:18,sizeZ:11,powered:0b,ignoreEntities:0b,posX:0,posY:1,posZ:0,metadata:"",mirror:"NONE",seed:0L,author:"?",rotation:"NONE",mode:"LOAD",integrity:1.0f,showair:0b,showboundingbox:1b}
function bs.core:menu/creator/place_system/validation
