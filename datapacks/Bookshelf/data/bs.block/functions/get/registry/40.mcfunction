...
execute if block ~ ~ ~ #bs.block:has_state[signal_fire=false] run data modify storage bs:out block.iterable_properties[{name:"signal_fire"}].options[{value:"false"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[signal_fire=true] run data modify storage bs:out block.iterable_properties[{name:"signal_fire"}].options[{value:"true"}].selected set value 1b
