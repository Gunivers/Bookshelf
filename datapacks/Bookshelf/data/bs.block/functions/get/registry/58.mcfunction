...
execute if block ~ ~ ~ #bs.block:has_state[cracked=false] run data modify storage bs:out block._[{name:"cracked"}].options[{value:"false"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[cracked=true] run data modify storage bs:out block._[{name:"cracked"}].options[{value:"true"}].selected set value 1b
