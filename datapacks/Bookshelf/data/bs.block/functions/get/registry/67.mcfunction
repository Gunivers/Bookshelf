...
execute if block ~ ~ ~ #bs.block:has_state[enabled=true] run data modify storage bs:out block.iterable_properties[{name:"enabled"}].options[{value:"true"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[enabled=false] run data modify storage bs:out block.iterable_properties[{name:"enabled"}].options[{value:"false"}].selected set value 1b
