...
execute if block ~ ~ ~ #bs.block:has_state[mode=compare] run data modify storage bs:out block.iterable_properties[{name:"mode"}].options[{value:"compare"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[mode=subtract] run data modify storage bs:out block.iterable_properties[{name:"mode"}].options[{value:"subtract"}].selected set value 1b
