...
execute if block ~ ~ ~ #bs.block:has_state[extended=false] run data modify storage bs:out block.iterable_properties[{name:"extended"}].options[{value:"false"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[extended=true] run data modify storage bs:out block.iterable_properties[{name:"extended"}].options[{value:"true"}].selected set value 1b
