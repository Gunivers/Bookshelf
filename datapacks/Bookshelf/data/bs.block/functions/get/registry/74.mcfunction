...
execute if block ~ ~ ~ #bs.block:has_state[has_book=false] run data modify storage bs:out block.iterable_properties[{name:"has_book"}].options[{value:"false"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[has_book=true] run data modify storage bs:out block.iterable_properties[{name:"has_book"}].options[{value:"true"}].selected set value 1b
