...
execute if block ~ ~ ~ #bs.block:has_state[eye=false] run data modify storage bs:out block._[{name:"eye"}].options[{value:"false"}].selected set value 1b
execute if block ~ ~ ~ #bs.block:has_state[eye=true] run data modify storage bs:out block._[{name:"eye"}].options[{value:"true"}].selected set value 1b
