...
execute store result score @s glib.var2 run data get entity @e[tag=glib.id.match,limit=1,sort=nearest] Pos[1] 1000

scoreboard players operation @s glib.var2 -= @s glib.var

### DEBUG
#tellraw @a[tag=Debug] ["",{"text":"-=[Debug Entity/Link/Imitate_Y]=-","color":"green"}]
#tellraw @a[tag=Debug] ["",{"text":"INPUT -> ","color":"gray"},{"text":"New Relative Y: ","color":"red"},{"score":{"name":"@s","objective":"glib.var2"}},{"text":".   Old Relative Y: ","color":"red"},{"score":{"name":"@s","objective":"glib.link.r.y"}}]
### END DEBUG

scoreboard players operation @s glib.var2 += @s glib.link.r.y
