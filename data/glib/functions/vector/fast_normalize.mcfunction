...
# Original path: glib:vector/normalize
...
### DEBUG
execute if score @s VectorX matches 2147483.. run tellraw @a[tag=glib.debug] ["",{"text":"[ERROR] VectorX too high, int overflow\n - glib:vector/fast_normalize","color":"green"},{"text":" - Entity: ","color":"dark_red"},{"selector":"@s","color":"dark_red"},{"text":" glib.debug.id: ","color":"dark_red"},{"score":{"name":"@s","objective":"glib.debug.id"},"color":"dark_red"}]
### END DEBUG
