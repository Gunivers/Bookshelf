...
execute store result entity @e[type=armor_stand,tag=Glib_Tmp_Collision_Detection,limit=1,sort=nearest] Pos[0] double 0.001 run scoreboard players operation @e[type=armor_stand,tag=Glib_Tmp_Collision_Detection,limit=1,sort=nearest] LocX += @s Var7

execute at @e[type=armor_stand,tag=Glib_Tmp_Collision_Detection,limit=1,sort=nearest] if block ~ ~ ~ #gunivers-lib:no_collision run tag @s remove CollisionX
